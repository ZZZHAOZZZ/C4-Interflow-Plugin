...
set "aac-root-namespace=ContentManagementSystem"
...
if %redraw-all%==TRUE (
%cli-output-dir%\%cli-exe% draw-diagrams --interfaces %aac-root-namespace%.SoftwareSystems.*.Containers.*.Interfaces.* --interfaces %aac-root-namespace%.SoftwareSystems.*.Containers.*.Components.*.Interfaces.* --interfaces %aac-root-namespace%.SoftwareSystems.*.Interfaces.* --business-processes %aac-root-namespace%.BusinessProcesses.* --levels-of-details context container component --aac-reader-strategy "%aac-reader-strategy%" --aac-input-paths "%aac-input-paths%" --output-dir "%diagrams-dir%" --formats png svg 
) else (
